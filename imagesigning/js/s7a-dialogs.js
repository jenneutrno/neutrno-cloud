 ...
 * ownCloud
 *
 * @author Bartek Przybylski, Christopher Schäpers, Thomas Tanghus, Carlo Rimondi
...
var S7Adialogs = {
	...
	*/
	info:function(text, title, callback, modal) {
		this.message(text, title, 'info', S7Adialogs.OK_BUTTON, callback, modal);
	},
    /**
     * displays waiting dialog
     * @param text content of dialog
     * @param title dialog title
     * @param callback which will be triggered when user presses OK
     * @param modal make the dialog modal
     */
    waiting:function(text, title, callback, modal) {
        var htmlText = '<div>'+text+'</div>' +
            '<div>' +
            '<img class="waiting-circle" src="'+OC.imagePath('imagesigning', 'ico-loading.gif')+'" />' +
            '</div>';
        var diagId = '#oc-dialog-' + S7Adialogs.dialogsCounter + '-content-s7a';
        this.message(htmlText, title, 'info', S7Adialogs.CANCEL_BUTTON, callback, modal, true);
        return diagId;
    },
     ...
     * @param id dialog id
     */
    closeDiag:function(id) {
        if (id !== null) $(id).ocdialog('close');
    },
	...
	*/
	...
	prompt: function (text, title, callback, modal, name, password) {
		return $.when(this._getMessageTemplate()).then(function ($tmpl) {
			var dialogName = 'oc-dialog-' + S7Adialogs.dialogsCounter + '-content';
	...
	*/
	filepicker:function(title, callback, multiselect, mimetypeFilter, modal, startdir) {
		var self = this;
		...
		$.when(this._getFilePickerTemplate()).then(function($tmpl) {
			...
			self.$filePicker = $tmpl.octemplate({
				dialog_name: dialogName,
				title: title,
				emptytext: t('core', 'No folder in here')
	...
	*/
	message:function(content, title, dialogType, buttons, callback, modal, allowHtml) {
		return $.when(this._getMessageTemplate()).then(function($tmpl) {
			var dialogName = 'oc-dialog-' + S7Adialogs.dialogsCounter + '-content-s7a';
			var dialogId = '#' + dialogName;
			var $dlg = $tmpl.octemplate({
				dialog_name: dialogName,
				title: title,
			...
			var buttonlist = [];
			switch (buttons) {
			case S7Adialogs.YES_NO_BUTTONS:
				buttonlist = [{
					text: t('core', 'No'),
					click: function(){
						if (callback !== undefined) {
